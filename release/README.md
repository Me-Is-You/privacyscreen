...
- **Venetian Blinds**: Add optional horizontal gradient stripes to the vignette for a realistic 3D privacy-blind effect.
- **Realistic Lens Vignette**: New `lens` falloff function that simulates real camera lens vignetting with smoother edge transitions.
- **Advanced Pixel Patterns**: `mesh` (grid) and `dots` (circular dot matrix) patterns for more realistic physical privacy filters.
- **Dynamic Radius Control**: Adjust the size of the clear center area (`--radius`) for fine-tuned privacy coverage.
...
| `-t, --type` | The mathematical function for the fall-off | `smootherstep` | `power`, `exponential`, `gaussian`, `smootherstep`, `lens` |
...
| `--radius` | Clear center area radius factor | `1.0` | Any float |
...
| Flag | Description | Default | Values |
|------|-------------|---------|--------|
| `-p, --pattern` | The pattern for masking | `checkerboard` | `checkerboard`, `vertical`, `horizontal`, `diagonal`, `mesh`, `dots` |
